...
def analyse_file_by_hash(api_key, hash_code):
    ...
    try:
        ...
        # Extract meaningful name, fallback to hash_code if not available
        meaningful_name = json_response.get('data', {}).get('attributes', {}).get('meaningful_name', hash_code)

        # Sanitize the filename
        sanitized_name = "".join(c for c in meaningful_name if c.isalnum() or c in (' ', '.', '_')).rstrip()
        if not sanitized_name:
            sanitized_name = hash_code  # Fallback if sanitized_name is empty
        ...
        file_name = f"{sanitized_name}.analysis_response.json"
